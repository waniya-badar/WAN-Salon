...
private://encapsulation
	...
	string card;
public:
	bool pre_book;//if prebook 0 else 1
	static int count;
...
public:
	...
	void savingdatain_file() {
		fstream filing("Salon.txt", ios::app);
		if (filing) {
			filing << arr[Customer::count - 1]->name << "\t\t" << arr[Customer::count - 1]->no_of_services << "\t" << arr[Customer::count - 1]->gettotalamount() << endl;

		}
		filing.close();
	}
	void readingdata_fromfile() {
		fstream f;
		f.open("Salon.txt", ios::in);
		string line;
		cout << "Name\tNO of Services\tTotal Amount" << endl;
...
string MakeupTimeSlotBooked[8][6][3];//8 makeup artists, 6 workingdays and 3 timeslots
...
int main() {
	...
	int k = -1;
	Salon obj2;
	fstream fil("Salon.txt", ios::out | ios::trunc);
	fil.close();
	do {
		cout << "Select\n1 : Inquiry\n2 : Appointment \n3 : total amount and admin details\n4 : Pre-booked  Appointments \n5 : Execution" << endl;
		cin >> a;
		if (a == 1) {
			services* s = new Schedule();//polymorphism
			s->print_inquiry();
		}
		else if (a == 2) {
			k++;
			...
			cout << "Select 1 : pre-booked appointment\nSelect 2 :To proceed next" << endl;
			int select;
			cin >> select;
			if (select == 1) {
				cout << "Good Bye!" << endl;
				temp.pre_book = 0;//no services done
			}
			else {
				cout << "Proceed to counter number 2" << endl;//services
				temp.pre_book = 1;
			}
			obj[k] = temp;//For saving data of customer data is being copied in an array;
			obj2.savingdatain_file();
		}
		else if (a == 3) {
			...
			string tt;
			cin >> tt;
			string compare = name2 + "\t\t" + no + "\t" + tt;
			fstream f;
			f.open("Salon.txt", ios::in);
			string line;
			int num = 0;
			int veri = 0;//if 1 successfull
			while (getline(f, line)) {
				if (line == compare && obj[num].pre_book == 0) {
					cout << "Verification successfull!\nPlease proceed next" << endl;
					veri = 1;
					obj[num].pre_book = 1;
					break;
			...
			if (veri == 0) {
				cout << "Sorry your details donot match in our system" << endl;
			}
			f.close();
		}
	} while (a != 5);
